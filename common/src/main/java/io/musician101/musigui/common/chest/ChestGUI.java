...
public abstract class ChestGUI<ClickType, Inventory, PluginContainer, Player, ItemStack, Name, InventoryView, InventoryClose> {
    ...
    public final void setButton(int slot, @Nonnull ItemStack itemStack, @Nonnull ClickType clickType, @Nonnull Consumer<Player> action) {
        setButton(slot, itemStack, Map.of(clickType, action));
    }

    public final void setButton(int slot, @Nonnull ItemStack itemStack, @Nonnull Map<ClickType, Consumer<Player>> actions) {
        buttons.removeIf(g -> g.getSlot() == slot);
